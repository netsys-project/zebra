...
class SimpleApp {
 public:
   ...
   * is communicated
   */
  explicit SimpleApp(int app_id, Postoffice *off);
...
inline SimpleApp::SimpleApp(int app_id, Postoffice *off) : SimpleApp() {
  using namespace std::placeholders;
  office = off;
  obj_ = new Customer(office, app_id, std::bind(&SimpleApp::Process, this, _1));
}
...
inline int SimpleApp::Request(int req_head, const std::string& req_body, int recv_id) {
  ...
  // send
  for (int r : office->GetNodeIDs(recv_id)) {
    msg.meta.recver = r;
    office->van()->Send(msg);
  }
  return ts;
