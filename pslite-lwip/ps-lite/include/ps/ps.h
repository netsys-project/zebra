...
inline int NumWorkers() { return Postctl::Get()->num_workers(); }
/** \brief Returns the number of server nodes */
inline int NumServers() { return Postctl::Get()->num_servers(); }
///** \brief Returns true if this node is a worker node */
//inline bool IsWorker() { return Postctl::Get()->is_worker(); }
/** \brief Returns true if this node is a server node. */
inline bool IsServer() {
	return Postctl::Get()->has_server() > 0 ? true : false;
}
/** \brief Returns true if this node is a scheduler node. */
//inline bool IsScheduler() { return Postoffice::Get()->is_scheduler(); }

inline int NumTasks() { return Postctl::Get()->num_tasks(); }
inline int LocalWorkers() { return Postctl::Get()->local_workers(); }

inline int Init() {
	return Postctl::Get()->init();
}

inline int Start(int argc, char *argv[]) {
	return Postctl::Get()->startTasks(argc, argv);
}
 ...
 */
//inline int MyRank() { return Postctl::Get()->GetOffice()->my_rank(); }
/**
