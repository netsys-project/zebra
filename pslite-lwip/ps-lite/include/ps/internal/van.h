...
class Van {
 public:
  ...
  virtual int Init(const char *ip, const char *gw, const char *mask) { return 0; };
  /**
   * \brief create Van
   * \param type zmq, socket, ...
   */
  static Van* Create(const std::string& type, Postoffice *po);
  /** \brief constructer, do nothing. use \ref Start for real start */
  Van(Postoffice *po) { office = po; }
 ...
 protected:
   ...
   */
  virtual int Connect(const Node& node) = 0;
  /**
   ...
   */
  virtual int RecvMsg(Message* msg, bool is_data) = 0;
  /**
   * \brief send a mesage
 ...
 private:
  /** thread function for receving control message */
  void Receiving();
  /** thread function for receving data message */
  void ReceivingData();
  ...
  int num_servers_ = 0;
  int num_workers_ = 0;
  /** the thread for receiving control messages */
  std::unique_ptr<std::thread> receiver_thread_;
  /** the thread for receiving data messages */
  std::unique_ptr<std::thread> data_receiver_thread_;
