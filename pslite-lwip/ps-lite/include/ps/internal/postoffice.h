...
* \param t timeout in sec
  ...
  void SetThread(int t) { tid = t; }
  int gettid(void) { return tid; }

  Postoffice(int servers, int workers, unsigned r, int v);
  ~Postoffice() { if (van_) { delete van_; van_ = NULL; } }
 private:
...
class Posttask {
	public:
		Posttask(int s, int w, unsigned r, int v);
		~Posttask() {}
...
//		static void taskthread(void *arg);
		...
		void GetTaskList(const char *str);
		void InsertTask(unsigned task, unsigned allow_dup);
		int local_workers() const { return local_workers_; }
		int num_tasks() const { return tasklist.size(); }
		/** \brief Returns the number of worker nodes */
		int num_workers() const { return num_workers_; }
		/** \brief Returns the number of server nodes */
		int num_servers() const { return num_servers_; }
		int has_server() const { return local_servers_; }
		/** \brief Returns the verbose level. */
		int verbose() const { return verbose_; }

		void addThread(int id, Postoffice *po);
		int startTasks(int argc, char *argv[]);

//		Postoffice *GetOffice(void);
	...
	private:
		...
		~Postctl();
  		...
  		std::vector<Posttask *> tasklist;
		std::map<int, Postoffice *> taskmap;
		pthread_mutex_t map_lock;
		int local_workers_, num_workers_;
		int local_servers_, num_servers_;
		int verbose_;
  		std::shared_ptr<Environment> env_ref_;
};

extern int is_scheduler_ready, is_server_ready;
/** \brief verbose log */
#define PS_VLOG(x) LOG_IF(INFO, x <= Postctl::Get()->verbose())
}  // namespace ps
