...
namespace zmq
{
    ...
    private:
		...
		public:
			...
			size_t outsize;
			i_encoder *encoder;
			unsigned char *hotoutpos;
			size_t hotoutsize;
			i_encoder *hotencoder;
			uint16_t id;

			outctl(uint16_t id_ = UINT16_MAX) :
					outpos(NULL), outsize(0), encoder(NULL),
					hotoutpos(NULL), hotoutsize(0), hotencoder(NULL),
					id(id_) {};
		};

		std::vector<outctl> outputs;
