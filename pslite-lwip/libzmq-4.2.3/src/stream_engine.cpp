				...
				session_base_t *session_)
{
	...
	if (socket->get_localid() == SCHEDULAR_ID ||
					socket->get_remoteid() == SCHEDULAR_ID) {
		is_ctl = true;
	}
...
//		selector->rm_fd(lwip_handle);
    ...
    if (!insize) {
        ...
        if (rc == 0) {
            ...
            errno = EPIPE;
//			fprintf(stderr, "[%s][%d]: connection is closed by peer\n",
//							__FILE__, __LINE__);
//			selector->rm_fd(lwip_handle);
...
//	if (insize > 0) {
//		fprintf(stdout, "[%s][%d]: receive %lu data\n",
//						__FILE__, __LINE__, insize);
//	}
    while (insize > 0) {
        rc = decoder->decode (inpos, insize, processed);
        zmq_assert (processed <= insize);
//		fprintf(stdout, "[%s][%d]: process %lu data\n",
//						__FILE__, __LINE__, processed);
        inpos += processed;
        insize -= processed;
        if (rc == 0 || rc == -1)
            break;
    ...
    if (rc == -1) {
        ...
        input_stopped = true;
        reset_pollin_lwip (handle);
//		selector->reset_pollin(lwip_handle);
    }
...
void zmq::stream_engine_t::data_out_event()
{
	...
	if (!is_id_set && socket->get_localid() != UINT16_MAX) {
		lwip_setlocalid(s, socket->get_localid());
		is_id_set = true;
		fprintf(stdout, "[%s][%d]: set local id %u\n",
						__FILE__, __LINE__, socket->get_localid());
	}
...
//							__FILE__, __LINE__, tx_msg.get_remote_id(),
//							tx_msg.size(), tx_msg.flags());
			if (tx_msg.flags() & msg_t::netml_data) {
				fprintf(stderr, "[%s][%d]: control path send data message?\n",
								__FILE__, __LINE__);
				continue;
			}
			encoder->load_msg(&tx_msg);
...
bool zmq::stream_engine_t::can_handle_data()
{
	for (unsigned i = 0; i < num_target; i++) {
		if (outputs[i].outsize >= (size_t)out_batch_size) {
			return false;
		}
		if (outputs[i].hotoutsize >= (size_t)out_batch_size) {
			return false;
		}
	}
	return true;
...
bool zmq::stream_engine_t::has_data()
{
	for (unsigned i = 0; i < num_target; i++) {
		if (outputs[i].outsize > 0)
			return true;
		if (outputs[i].hotoutsize > 0)
			return true;
	}
	return false;
}
...
//						__FILE__, __LINE__, socket->get_localid());
	...
	if (!has_data()) {
		for (i = 0; i < num_target; i++) {
			outputs[i].outpos = NULL;
			outputs[i].outsize = outputs[i].encoder->encode(
							&outputs[i].outpos, 0);
			outputs[i].hotoutpos = NULL;
			outputs[i].hotoutsize = outputs[i].hotencoder->encode(
							&outputs[i].hotoutpos, 0);
		}

		while (can_handle_data()) {
			if ((this->*next_msg)(&tx_msg) == -1)
				break;
//			fprintf(stdout, "[%s][%d]: load data msg , target %u, size %lu, flags %x\n",
//							__FILE__, __LINE__,
//							tx_msg.get_remote_id(),
//							tx_msg.size(),
//							tx_msg.flags());
			if (!(tx_msg.flags() & msg_t::netml_data)) {
				fprintf(stderr, "[%s][%d]: data path send control message\n",
								__FILE__, __LINE__);
			}

			outctl *out = NULL;
			std::map<uint16_t, unsigned>::iterator it =
					targets.find(tx_msg.get_remote_id());
			if (it == targets.end()) {
				outputs.push_back(outctl(tx_msg.get_remote_id()));
				out = &(outputs[num_target]);

				out->encoder = new (std::nothrow)v2_encoder_t(out_batch_size);
				alloc_assert(out->encoder);

				out->hotencoder = new (std::nothrow)v2_encoder_t(out_batch_size);
				alloc_assert(out->hotencoder);

				targets.insert(std::pair<uint16_t, unsigned>(
						tx_msg.get_remote_id(), num_target));
				num_target++;
			}
			else {
				out = &outputs[it->second];
			}

			if (tx_msg.flags() & msg_t::netml_hotdata) {
				out->hotencoder->load_msg(&tx_msg);
				unsigned char *bufptr = out->hotoutpos + out->hotoutsize;
				size_t n = out->hotencoder->encode(&bufptr,
								out_batch_size - out->hotoutsize);

				zmq_assert(n > 0);
				if (out->hotoutpos == NULL)
					out->hotoutpos = bufptr;
				out->hotoutsize += n;
			}
			else {
				out->encoder->load_msg(&tx_msg);
				unsigned char *bufptr = out->outpos + out->outsize;
				size_t n = out->encoder->encode(&bufptr,
								out_batch_size - out->outsize);
...
//		}
	...
	for (unsigned i = 0; i < num_target; i++) {
		outctl *out = &outputs[i];

		if (out->outsize) {
			int nbytes = tcp_write(s, out->outpos, out->outsize, out->id);
		...
		if (out->hotoutsize) {
			int nbytes = tcp_write(s, out->hotoutpos, out->hotoutsize,
							out->id, true /* hot */);
			...
			out->hotoutpos += nbytes;
			out->hotoutsize -= nbytes;
		}
	}
}
#endif
...
void zmq::stream_engine_t::out_event()
{
	...
	if (is_ctl)
		return ctl_out_event();
	return data_out_event();
}
...
void zmq::stream_engine_t::out_event ()
{
	...
	for (unsigned i = 0; i < num_target; i++) {
		if (data_outputs[i].data_outsize > 0) {
			has_outsize = true;
			break;
		}
	}

	if (!outsize && !has_outsize) {
		...
		outpos = data_outpos = NULL;
		outsize = encoder->encode(&outpos, 0);
		data_outsize = data_encoder->encode(&data_outpos, 0);

		while ((outsize < (size_t)out_batch_size) &&
						(data_outsize < (size_t)out_batch_size)) {
			if ((this->*next_msg)(&tx_msg) == -1)
				break;
			fprintf(stdout, "[%s][%d]: load msg , target %u, size %lu, flags %x\n",
							__FILE__, __LINE__,
							tx_msg.get_remote_id(),
							tx_msg.size(),
							tx_msg.flags());
			if (tx_msg.flags() & msg_t::netml_data) {
				data_encoder->load_msg(&tx_msg);
				unsigned char *bufptr = data_outpos + data_outsize;
				size_t n = data_encoder->encode(&bufptr,
								out_batch_size - data_outsize);
				...
				encoder->load_msg(&tx_msg);
				unsigned char *bufptr = outpos + outsize;
				size_t n = encoder->encode(&bufptr,
								out_batch_size - outsize);
	...
	if (outsize) {
		int nbytes = tcp_write(s, outpos, outsize, 0 /* is_data */);
//		int nbytes = tcp_write(s, outpos, outsize);

		if (nbytes == -1) {
			reset_pollout_lwip(handle);
			return;
		}
//		fprintf(stdout, "[%s][%d]: write %d contrl msg\n",
//						__FILE__, __LINE__, nbytes);
	...
	if (data_outsize) {
		int nbytes = tcp_write(s, data_outpos, data_outsize, 1);
//		int nbytes = tcp_write(s, data_outpos, data_outsize);

		if (nbytes == -1) {
			reset_pollout_lwip(handle);
			return;
		}
		fprintf(stdout, "[%s][%d]: write %d data msg\n",
						__FILE__, __LINE__, nbytes);
		data_outpos += nbytes;
		data_outsize -= nbytes;
	}
...
void zmq::stream_engine_t::restart_output ()
{
    ...
    if (likely (output_stopped)) {
        set_pollout_lwip (handle);
//		selector->set_pollout(lwip_handle);
        output_stopped = false;
    }
...
void zmq::stream_engine_t::restart_input ()
{
    ...
    if (rc == -1)
        ...
        input_stopped = false;
//		selector->set_pollin(lwip_handle);
        set_pollin_lwip (handle);
        session->flush ();
...
bool zmq::stream_engine_t::handshake ()
{
    ...
    if (greeting_recv [0] != 0xff || !(greeting_recv [9] & 0x01)) {
        ...
        data_encoder = new (std::nothrow) v1_encoder_t (out_batch_size);
    ...
    if (greeting_recv [revision_pos] == ZMTP_2_0) {
        ...
        data_encoder = new (std::nothrow) v2_encoder_t (out_batch_size);
        alloc_assert (data_encoder);
